...
{
	ULocalPlayer* const Player = GetFirstGamePlayer();
	//GetFirstLocalPlayerController()->GetUniqueID()
	GEngine->AddOnScreenDebugMessage(-1, 10.f, FColor::Red, FString::Printf(TEXT("Finding player name %s"), *Player->GetName()));
...
void UNWGameInstance::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	if (OnlineSub)
	{
		...
		if (Sessions.IsValid())
		{
			...
			APlayerControllerBase* PlayerController = Cast<APlayerControllerBase>(GetFirstLocalPlayerController());
			if (PlayerController)
			{
